...
# Setup ────────────────────────────────────────────
import os, platform, time, socket, urllib.request, json, subprocess, sys, shutil, re
...
def descargar_paquete(nombre_paquete):
    ...
    # print(f"DEBUG: Intentando descargar JSON desde:\n {url_json}")
    ...
    if comando == "--help":
        ...
        while True:
            ...
            if comandologout == "login":
                ...
                # CORREGIDO: Usamos 'in' para buscar dentro del array 'users'
                if chooseuserlog in users:
                    working = True
                    passtologout = False
                    # Imprimimos el nombre que el usuario acaba de escribir
                    print(f"Sesión iniciada como {chooseuserlog}") 
                    break # ¡CLAVE! Rompe este bucle para volver al menú principal de la app
                    
                elif chooseuserlog.upper() == "G":
                    working = True
                    passtologout = False
                    print("Sesión iniciada como Invitado.")
                    break # Rompe este bucle para volver como invitado
        ...
        # Cambiado por un IF limpio en lugar del 'while' que tenías abajo
...
#           if sandbox_activo and not carpeta_padre.startswith(SANDBOX_ROOT):
        ...
        info = [
            "\033[34mWint\033[33mPy\033[0m OS 1.0.3 Main",
            "-------------------------",
